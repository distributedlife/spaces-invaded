...
LevelTwo = requirejs('invaders/level/two')
describe "level two", ->
  beforeEach ->
    level = new LevelTwo(100, 200, sound_manager)
  ...
  describe "when setting up all the things", ->
    ...
    it "should create a swarm of invaders going right", ->
      expect(stub_thing_factory.make_swarm).toHaveBeenCalled()

    it "should create a swarm of invaders going left", ->
      expect(stub_thing_factory.make_swarm).toHaveBeenCalled()

    it "should bind level failed to the tank die event", ->
      expect(tank.model.on_event).toHaveBeenCalledWith('die', level.level_failed)

    it "should bind swarm die to the swarm die event", ->
      expect(swarm.model.on_event).toHaveBeenCalledWith('die', level.swarm_die)
